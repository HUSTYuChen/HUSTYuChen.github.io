...
int main() {
	...
	float f1 = 3.14f; //一开始的数据为double 加f就防止其转化
	double d1 = 3.14; //一般只显示6位有效数值
...
int main() {
	...
	cout << (int)ch << endl;  //查看字符a对应的ASCII码  强制类型转换
...
> 总结1：数组名的命名规范与变量名命名规范一致，不要和变量重名 包括double这些变量名不可以一样
...
int main() {
	...
	//2、可以通过数组名获取到数组首地址 arr 16进制  &取址符号
...
> 总结1：直接打印数组名，可以查看数组所占内存的首地址 &取址符号
...
int main() {
	...
	p->score = 80; //指针通过 -> 操作符可以访问成员  (*p).age 也可以
...
//const使用场景
//将函数中的形参改为指针，可以减少内存空间，而且不会复制新的副本出来，这个时候可以用const
...
void allocateSpace(Teacher tArray[] , int len)
{
	...
	for (int i = 0; i < len; i++)
	{
		...
		for (int j = 0; j < 5; j++)
		{
			...
			tArray[i].sArray[j].score = rand() % 61 + 40;  //rand () % 60 = 0~59
		}
	}
}
